import { generateOTP } from '../src/functions';
...
describe('generate', () => {
    ...
    it('should throw an error if the length is a decimal number', () => {
        expect(() => generateOTP(4.2)).toThrow('OTP length must be a positive integer');
    });

    it('should throw an error if the length is zero', () => {
        expect(() => generateOTP(0)).toThrow('OTP length must be a positive integer');
    });

    it('should throw an error if the length is a negative number.', () => {
        expect(() => generateOTP(-10)).toThrow('OTP length must be a positive integer');
    });

    it('should throw an error if the length is less than the minimum allowed', () => {
        expect(() => generateOTP(MIN_LENGTH - 1)).toThrow(`OTP length must be between ${MIN_LENGTH} and ${MAX_LENGTH}`);
    });

    it('should throw an error if the length is greater than the maximum allowed', () => {
        expect(() => generateOTP(MAX_LENGTH + 1)).toThrow(`OTP length must be between ${MIN_LENGTH} and ${MAX_LENGTH}`);
    });
})